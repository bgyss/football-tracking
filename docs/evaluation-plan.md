...
| Cross-view identity | Zero false merges in the sample; at least 0.80 coverage of human-resolvable shared players | Precision and coverage reported together; unresolved matches remain explicit. Measured via `tracking-evaluation.json`'s `cross_shot` block, produced by `--reviewed-reference` carrying a `cross_shot_identity` map; today it reports `status: not_evaluated` because no such reference exists — see [docs/evidence/cross-shot-identity.md](evidence/cross-shot-identity.md). |
