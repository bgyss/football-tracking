...
def test_export_cvat_keeps_source_frames_and_inference_provenance(tmp_path) -> None:
    ...
    boundary = root.find("tag")
    assert boundary is not None and boundary.attrib == {"frame": "0", "label": "shot_boundary"}
    boundary_attributes = {node.attrib["name"]: node.text for node in boundary.findall("attribute")}
    assert boundary_attributes["end_frame_exclusive"] == "8"
    assert boundary_attributes["source_pts"] == str(pts[0])
...
def test_import_cvat_builds_reviewed_player_reference_and_keeps_other_labels(tmp_path) -> None:
    ...
    provenance = {
        ...
        "shots": {"shot-0": {"start_frame": 0, "end_frame": 8, "play_id": "play-1", "split": "development", "camera_label": "sideline"}},
        "observation_provenance": [{"shot_id": "shot-0", "source_frame": 3, "tracklet_id": "shot-0:t7", "raw_row": raw_row}],
    }
    provenance_path = tmp_path / "provenance.json"
    provenance_path.write_text(json.dumps(provenance), encoding="utf-8")
    xml_path = tmp_path / "reviewed.xml"
    xml_path.write_text(f'''<annotations>
      <tag frame="1" label="shot_boundary"><attribute name="shot_id">shot-0</attribute><attribute name="boundary">start</attribute><attribute name="end_frame_exclusive">8</attribute><attribute name="play_id">play-1</attribute><attribute name="split">development</attribute><attribute name="camera_label">sideline</attribute><attribute name="source_pts">{pts[1]}</attribute></tag>
      <track id="10" label="player" source="manual"><box frame="3" xtl="2" ytl="3" xbr="12" ybr="21" outside="0" occluded="1" keyframe="1">
        ...
        <attribute name="visibility">occluded</attribute><attribute name="detection_score">0.91</attribute>
      </box></track>
      <track id="11" label="official"><box frame="4" xtl="14" ytl="2" xbr="20" ybr="22" outside="0" occluded="0" keyframe="1"><attribute name="source_pts">{pts[4]}</attribute><attribute name="visibility">visible</attribute></box></track>
      <track id="12" label="football"><box frame="5" xtl="20" ytl="10" xbr="22" ybr="12" outside="0" occluded="0" keyframe="1"><attribute name="source_pts">{pts[5]}</attribute></box></track>
      <tag frame="3" label="timing_event"><attribute name="event">snap</attribute><attribute name="source_pts">{pts[3]}</attribute><attribute name="play_id">play-1</attribute><attribute name="correspondence_id">snap</attribute><attribute name="play_time_s">0</attribute></tag>
      <track id="13" label="calibration_landmark"><points frame="2" points="4,5" outside="0" occluded="0" keyframe="1"><attribute name="source_pts">{pts[2]}</attribute><attribute name="landmark_id">yardline:20:sideline:near</attribute><attribute name="field_x_yards">20</attribute><attribute name="field_y_yards">0</attribute><attribute name="role">fit</attribute></points></track>
    ...
    assert manifest["shots"]["shot-0"]["start_frame"] == 1
    ...
    assert reference["cross_shot_identity"]["shot-0"][player["track_id"]] == "anon-12"
